...
public:
	enum Colors {RED, GREEN, BLUE, YELLOW, WHITE, NONE};
	Quad();
	Quad(GLfloat, GLfloat, GLfloat,GLfloat, Colors);
	void Draw();
	void set_x(GLfloat);
	void set_y(GLfloat);
	void set_z(GLfloat);
	...
	void set_name(GLint name);
	GLint get_name();

	void set_x_speed(GLfloat);
	void set_y_speed(GLfloat);
	void set_z_speed(GLfloat);
...
Quad::Quad(GLfloat x, GLfloat y, GLfloat z, GLfloat height, Colors color)
{
	...
	static GLfloat white[]   = {1.0, 1.0, 1.0, 1.0};
	 ...
	 glTranslatef(x_pos_, y_pos_, 0);
	  glRotatef(angle_, 0.0f, 0.0f, 1.0f);
       ...
       glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, color_);
